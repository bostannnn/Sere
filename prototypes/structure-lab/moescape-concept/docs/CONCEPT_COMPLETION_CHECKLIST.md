...
- [ ] Sidebar tab order is final and approved: `Chats`, `Basics`, `Display`, `Lorebook`, `Voice`, `Scripts`, `Advanced`, `GameState`, `Share`.
- [ ] Primary navigation model is final: character-card home + chat runtime + right workspace sidebar.
- [ ] Character-scoped chat selector exists only in right sidebar `Chats`; left drawer `Recent Chats` is quick-open only.
- [ ] Home-to-chat entry rule is final: click character -> open last selected chat for that character.
...
- [ ] Migration map completed and approved.
- [ ] Component mapping completed (concept -> actual Svelte components).
- [ ] Slice plan created with estimated risk per slice.
