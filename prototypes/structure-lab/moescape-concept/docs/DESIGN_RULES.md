...
Chat runtime themes (`standard`, `waifu`, `mobilechat`, `cardboard`, `customHTML`) and custom background upload are intentionally **not implemented** in this prototype. `Display` settings cover color-scheme switching only. Theme/background must be implemented in the production migration slice, reusing the original app's settings keys and data paths — never prototype-only storage keys. Migration details and phasing are in `docs/MIGRATION_MAP.md` and `docs/MIGRATION_PLAN.md`.
