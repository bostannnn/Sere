...
Scope: Phase 3 Home + Global Drawer (`ui_shell_v2`) migration parity.

## Automated Gate Results

| Check | Result | Notes |
|---|---|---|
| `pnpm run check:ui-shell-smoke` | PASS | Includes `check-ui-shell-contract`, `check-chat-sidebar-contract`, `check-charconfig-rulebook-contract`, `ui-shell-runtime-smoke`, `global-launcher-runtime-smoke`, `home-character-directory-runtime-smoke`, `chat-runtime-smoke`, `chat-sidebar-runtime-smoke`, `chat-sidebar-integration-runtime-smoke`, `lorebook-runtime-smoke`. |
...
| Global drawer recent-chat quick-open routes to target chat | PASS | `src/lib/UI/GlobalLauncher.svelte` + `dev/global-launcher-runtime-smoke.test.ts` |
| Recent-chat quick-open fallback stability (missing IDs/index drift) | PASS | `src/lib/UI/GlobalLauncher.svelte` + `dev/global-launcher-runtime-smoke.test.ts` |
| Legacy Risuai brand/collapse controls removed from drawer | PASS | `src/lib/UI/GlobalLauncher.svelte` + `dev/global-launcher-runtime-smoke.test.ts` + `dev/check-ui-shell-contract.js` |
