...
> Deprecation note: **Comfy Commander plugin is superseded by the core Comfy Commander feature** (`/cw`, `/comfy`, Advanced settings). New installs should use core integration instead of plugin import.
...
### 6. Respect Color Schemes and Asset Size

- Read colors from `db.colorScheme` via `Risuai.getDatabase()` (themes do not provide color values).
- Prefer **inlay assets** (`saveInlayAsset`) for images to avoid embedding large base64 strings in chat history/exports.
