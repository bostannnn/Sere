...
function renderMarkdown(md:markdownit, data:string){
    ...
    data = data.replace(/\$\$(.*?)\$\$/gs, (
        ...
        try {
            ...
            parserLog('KaTeX render error:', error)
            return match
        }
    })
...
export function addMetadataToElement(data:string, modelShortName:string){
    ...
    parserLog('Encoded metadata:', encodedMetaCode.length, 'characters')
    parserLog('This requires at least', Math.ceil(encodedMetaCode.length / 32), '<p> tags to store')

    const d =  data.replace(/<p>/g, () => {
...
export async function hasher(data:Uint8Array){
    return Buffer.from(await crypto.subtle.digest("SHA-256", data as unknown as BufferSource)).toString('hex');
}
...
export function checkImageType(arr:Uint8Array | null | undefined):ImageType {
    if (!arr || arr.length < 2) {
        return "Unknown";
    }
    ...
    const isGIF = arr.length >= 6 && arr[0] === 0x47 && arr[1] === 0x49 && arr[2] === 0x46 && arr[3] === 0x38 && (arr[4] === 0x37 || arr[4] === 0x39) && arr[5] === 0x61;
    const isBMP = arr[0] === 0x42 && arr[1] === 0x4D;
    const isAVIF = arr.length >= 12 && arr[4] === 0x66 && arr[5] === 0x74 && arr[6] === 0x79 && arr[7] === 0x70 && arr[8] === 0x61 && arr[9] === 0x76 && arr[10] === 0x69 && arr[11] === 0x66;
    const isWEBP = arr.length >= 12 && arr[0] === 0x52 && arr[1] === 0x49 && arr[2] === 0x46 && arr[3] === 0x46 && arr[8] === 0x57 && arr[9] === 0x45 && arr[10] === 0x42 && arr[11] === 0x50;
...
const legacyBlockMatcher = (p1:string,_matcherArg:matcherArg) => {
    const bn = p1.indexOf('\n')
...
function blockStartMatcher(p1:string,_matcherArg:matcherArg):{type:blockMatch,type2?:string,funcArg?:string[],mode?:string}{
...
function blockEndMatcher(p1:string,type:{type:blockMatch,type2?:string,mode?:string},_matcherArg:matcherArg):string{
    ...
    if (node.nodeType === Node.TEXT_NODE) {
        ...
        if (text) {
                ...
                if (markdown !== text) {
                    ...
                    span.replaceChildren(document.createRange().createContextualFragment(sanitizedMarkdown));
