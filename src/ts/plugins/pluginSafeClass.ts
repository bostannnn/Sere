...
const pluginSafeLog = (..._args: unknown[]) => {};
...
export const SafeDocument = {
    ...
    createElement: (tagName: string): HTMLElement => {
        pluginSafeLog('Creating element:', tagName);
    ...
    createElementNS: (namespaceURI: string, qualifiedName: string): Element => {
        pluginSafeLog('Creating namespaced element:', qualifiedName);
    ...
    addEventListener: (type: string, listener: EventListenerOrEventListenerObject, options?: boolean | AddEventListenerOptions): void => {
        ...
        if(!allowedEvents.includes(type)) {
            pluginSafeLog(`Event type '${type}' is not allowed in plugin context.`);
            return;
        }
