...
import { additionalChatMenu, additionalFloatingActionButtons, additionalHamburgerMenu, additionalSettingsMenu, DBState, pluginAlertModalStore, selectedCharID, type MenuDef } from "src/ts/stores.svelte";
import { v4 } from "uuid";
...
class SafeElement {
    ...
    public setInnerHTML(value: string) {
        ...
        this.#element.replaceChildren(document.createRange().createContextualFragment(san));
    }
    ...
    #eventIdMap = new Map<string, EventListener>()

    public async addEventListener(type:string, listener: (event: unknown) => void, options?: boolean | AddEventListenerOptions):Promise<string> {
        ...
        if(allowedDocumentEventListeners.includes(type)){
            const modifiedListener: EventListener = (event) => {
...
class SafeDocument extends SafeElement {
    ...
    createElement(tagName: string): SafeElement {
        if(!tagWhitelist.includes(tagName.toLowerCase())) {
            pluginV3Log(`Creation of <${tagName}> elements is restricted. Creating a <div> instead.`);
            tagName = 'div';
        }
        if(tagName.toLowerCase() === 'a'){
            pluginV3Log(`<a> can be created but href attribute cannot be set directly for security reasons. Use .createAnchorElement(href: string) to create safe anchor elements.`);
        }
    ...
    createAnchorElement(href: string): SafeElement {
        ...
        try {
            ...
            pluginV3Log(`Invalid URL provided for anchor element: ${href}. Setting href to '#' instead.`);
...
type PluginUnloadCallback = () => void | Promise<void>
const pluginUnloadCallbacks: Map<string, PluginUnloadCallback[]> = new Map();

const addPluginUnloadCallback = (pluginName: string, callback: PluginUnloadCallback) => {
...
const unloadV3Plugin = async (pluginName: string) => {
    ...
    try {
        ...
        pluginV3Log(`Error terminating plugin ${pluginName}:`, error);
    }
}
...
const makeRisuaiAPIV3 = (iframe:HTMLIFrameElement,plugin:RisuPlugin) => {
    ...
    return {
        ...
        registerCommandHandler: oldApis.registerCommandHandler,
        unregisterCommandHandler: oldApis.unregisterCommandHandler,
        runMainLLM: oldApis.runMainLLM,
        setPluginProgress: oldApis.setPluginProgress,
        ...
        saveInlayAsset: oldApis.saveInlayAsset,
        ...
        getDatabase: async (includeOnly:string[]|'all' = 'all') => {
            ...
            for(const key of allowedDbKeys){
                ...
                liteDB[key] = $state.snapshot(db[key]);
            }
            return liteDB;
        },
        ...
        setCharacterToIndex: (index:number, char:unknown) => {
            const db = DBState.db
            const charIds = Object.keys(db.characters);
            const charId = charIds[index];
            if(charId){
        ...
        setChatToIndex: (characterIndex:number, chatIndex:number, chat:unknown) => {
            const db = DBState.db
            const charIds = Object.keys(db.characters);
            const charId = charIds[characterIndex];
            if(charId){
        ...
        registerButton: (
            ...
            const { name, icon, iconType } = arg;
            const location = arg.location || 'action';
        ...
        log: (message:string) => {
            pluginV3Log(`[RisuAI Plugin: ${plugin.name}] ${message}`);
        },
        ...
        requestPluginPermission: (permission:string) => {
            if (permission === 'fetchLogs' || permission === 'db' || permission === 'mainDom') {
                return getPluginPermission(plugin.name, permission);
            }
...
export async function executePluginV3(plugin:RisuPlugin){
    if (typeof plugin.script !== 'string' || plugin.script.length === 0) {
        pluginV3Log(`[RisuAI Plugin: ${plugin.name}] Missing script. Skipping load.`);
        return;
    }
    const safetyResult = await checkCodeSafety(plugin.script);
    if (!safetyResult.isSafe) {
        pluginAlertModalStore.errors = safetyResult.errors;
        pluginAlertModalStore.open = true;
        pluginV3Log(`[RisuAI Plugin: ${plugin.name}] Blocked by plugin safety checks.`);
        return;
    }

    const alreadyRunning = v3PluginInstances.find(p => p.name === plugin.name);
    if(alreadyRunning){
        pluginV3Log(`[RisuAI Plugin: ${plugin.name}] Plugin is already running. Skipping load.`);
        return;
    }
    ...
    host.run(iframe, safetyResult.modifiedCode);
    pluginV3Log(`[RisuAI Plugin: ${plugin.name}] Loaded API V3 plugin.`);
}
...
globalThis.__debugV3Plugin = (code: string|(() => unknown), pluginName: string = '') => {
