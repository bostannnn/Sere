...
interface RisuaiPluginAPI {
    ...
    /**
     * Registers a custom slash-command handler.
     * The handler can return a string to set the pipe value, or undefined to allow built-in handling.
     */
    ...
    /**
     * Unregisters a custom slash-command handler.
     */
    ...
    /**
     * Runs the main LLM with a simple user prompt and returns the raw text response.
     */
    runMainLLM(
        prompt: string,
        options?: {
    ...
    /**
     * Controls a small plugin progress bar near the input area.
     */
    setPluginProgress(
        active: boolean,
        label?: string,
    ...
    /**
     * Saves an inlay asset (stored separately from assets)
     * @param data - Asset data
     * @param fileName - Optional file name
     * @returns Inlay asset id
     */
    saveInlayAsset(data: any, fileName?: string): Promise<string>;
