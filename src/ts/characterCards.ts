...
import { type character, setDatabase, type customscript, type loreSettings, type loreBook, type triggerscript, importPreset, type groupChat, getDatabase, setDatabaseLite } from "./storage/database.svelte"
...
export async function importCharacterProcess(f:{
    ...
    if(f.name.endsWith('charx') || f.name.endsWith('jpg') || f.name.endsWith('jpeg')){
        characterCardsLog('reading charx')
        alertStore.set({
            type: 'wait',
            msg: 'Loading... (Reading)'
        })
        ...
        if(f.data instanceof ReadableStream){
            ...
            const rsp = new Response(f.data as BodyInit)
...
async function importCharacterCardSpec(card:CharacterCardV2Risu|CharacterCardV3, img?:Uint8Array, mode:'hub'|'normal' = 'normal', assetDict:{[key:string]:string} = {}, overrideLorebook: loreBook[] = null):Promise<boolean>{
    ...
    characterCardsLog(`Importing ${card.spec}, mode is ${mode}`)
