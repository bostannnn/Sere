...
export async function encode(data:string):Promise<(number[]|Uint32Array|Int32Array)>{
    ...
    let result: number[] | Uint32Array | Int32Array | undefined = undefined;

    if(db.aiModel === 'openrouter'){
...
let tokenizersTokenizer:Tokenizer | null = null
let tokenizersType:tokenizerType | null = null
let lastTikModel = 'cl100k_base'
let tikParserInitQueue: Promise<void> = Promise.resolve()
let tiktokenInitPromise: Promise<void> | null = null
let tiktokenCtor: (new (bpeRanks: string, specialTokens: Record<string, number>, patStr: string) => Tiktoken) | null = null

const googleCloudTokenizedCache = new Map<string, number>()
let webTokenizerFallbackWarned = false

async function ensureTiktokenInitialized() {
    if (tiktokenCtor) {
        return tiktokenCtor
    }
    ...
    if (!tiktokenInitPromise) {
        tiktokenInitPromise = (async () => {
            const [{ init, Tiktoken }, wasmModule] = await Promise.all([
                import('@dqbd/tiktoken/lite/init'),
                import('@dqbd/tiktoken/lite/tiktoken_bg.wasm?url'),
            ])
            ...
            tiktokenCtor = Tiktoken as new (bpeRanks: string, specialTokens: Record<string, number>, patStr: string) => Tiktoken
        })().catch((error) => {
            tiktokenCtor = null
            tiktokenInitPromise = null
            throw new Error(`tiktoken WASM initialization failed: ${error instanceof Error ? error.message : String(error)}`)
        })
    }
...
async function rebuildTikParser(model: 'cl100k_base' | 'o200k_base') {
    const TiktokenCtor = await ensureTiktokenInitialized()
    if (model === 'cl100k_base') {
        const cl100k_base = await import("@dqbd/tiktoken/encoders/cl100k_base.json")
        return new TiktokenCtor(
...
async function tokenizeGoogleCloud(text:string) {
    ...
    if(googleCloudTokenizedCache.has(cacheKey)){
        const count = googleCloudTokenizedCache.get(cacheKey)
...
let gemmaTokenizer:GemmaTokenizer | null = null
...
async function tikJS(text:string, model='cl100k_base') {
    const targetModel = model === 'o200k_base' ? 'o200k_base' : 'cl100k_base'

    tikParserInitQueue = tikParserInitQueue.then(async () => {
        if (tikParser && lastTikModel === targetModel) {
            return
        }
        tikParser?.free()
        tikParser = await rebuildTikParser(targetModel)
        lastTikModel = targetModel
    }, async () => {
        tikParser?.free()
        tikParser = await rebuildTikParser(targetModel)
        lastTikModel = targetModel
    ...
    await tikParserInitQueue

    if (!tikParser) {
        throw new Error('tiktoken parser is not available after initialization')
    }
    return tikParser.encode(text)
}

async function _geminiTokenizer(text:string) {
    const db = getDatabase()
...
async function tokenizeWebTokenizers(text:string, type:tokenizerType) {
    try {
        ...
        if (!webTokenizerFallbackWarned) {
            webTokenizerFallbackWarned = true
            tokenizerLog('[Tokenizer] Web tokenizer module failed to load. Falling back to cl100k_base.', error)
        }
        try {
            return await tikJS(text, 'cl100k_base')
        } catch {
            // Last-resort estimator keeps UI/chat functional when tokenizer modules fail.
            const estimated = Math.max(1, Math.ceil(text.length / 4))
            return new Uint32Array(estimated)
        }
    }
}
...
export async function strongBan(data:string, bias:{[key:number]:number}) {
    const cached = localStorage.getItem('strongBan_' + data)
    if(cached !== null){
        return JSON.parse(cached)
    }
    const _performace = performance.now()
    const _length = Object.keys(bias).length
    const charAlt = [
        data,
        data.trim(),
