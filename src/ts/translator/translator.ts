...
const translatorLog = (..._args: unknown[]) => {};
...
async function translateMain(text:string, arg:{from:string, to:string, host:string, translatorNote?:string}){
    ...
    const translatedRows = Array.isArray(res[0]) ? (res[0] as Array<[string, ...unknown[]]>) : [];
    const result = translatedRows.map((s: [string, ...unknown[]]) => s[0]).filter(Boolean).join('').replace(/\* ([^*]+)\*/g, '*$1*').replace(/\*([^*]+) \*/g, '*$1*');
    return result
}
...
export async function translateHTML(html: string, reverse:boolean, charArg:simpleCharacterArgument|string = '', chatID:number, regenerate = false): Promise<string> {
    ...
    const db = getDatabase()
    const DoingChat = get(isDoingChat)
    if(DoingChat){
        if(isExpTranslator()){
    ...
    if(db.translatorType === 'llm'){
        ...
        const from = db.translatorInputLanguage || 'en'
    ...
    async function translateTranslationChunks(force:boolean = false, additionalChunkLength = 0){
        ...
        translatorLog(split.length, currentChunk.chunks.length)
        ...
        if(nodeText.trim().length !== 0){
            if(needSuperChunkedTranslate()){
                const prm = new Promise<string>((resolve) => {
                    translateTranslationChunks(false, nodeText.length)
                    translationChunks[translationChunks.length-1].resolvers.push(resolve)
                    translationChunks[translationChunks.length-1].chunks.push(nodeText)
                })
            ...
            newNode.replaceChildren(document.createRange().createContextualFragment(DOMPurify.sanitize(processedTranslated)));
...
async function translateLLM(text:string, arg:{to:string, from:string, regenerate?:boolean,translatorNote?:string}):Promise<string>{
    ...
    let translatorNote = ""
    translatorLog(arg.translatorNote)
