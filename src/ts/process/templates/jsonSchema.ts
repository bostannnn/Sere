...
export function extractJSON(data:string, format:string): string{
    const extract = (data:unknown, format:string): string => {
        try {
            ...
            const current = (data as Record<string, unknown>)[fp[0]]
            ...
            if(current === undefined){
                ...
                return extract(current, fp.slice(1).join('.'))
            }
            else if(Array.isArray(current)){
                const index = parseInt(fp[1])
                return extract(current[index], fp.slice(1).join('.'))
            }
            else{
                return `${current ?? ''}`
            }   
