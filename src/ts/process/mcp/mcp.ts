...
export async function callMCPTool(methodName:string, args:unknown):Promise<RPCToolCallContent[]> {
...
export async function callTool(methodName:string, args:unknown) {
...
export async function decodeToolCall(text:string):Promise<toolCallData|undefined> {
    ...
    const [callId, _callName] = text.split('\uf100');
    if(!callId) {
        return undefined;
    }
