...
export class GoogleSearchClient extends MCPClientLike {
    ...
    async callTool(toolName: string, args: unknown): Promise<RPCToolCallContent[]> {
        try {
            switch (toolName) {
                case "google_search":
                    return await this.performWebSearch(args as WebSearchArgs);
                case "google_search_images":
                    return await this.performImageSearch(args as ImageSearchArgs);
                default:
    ...
    private async performWebSearch(args: WebSearchArgs): Promise<RPCToolCallContent[]> {
        ...
        try {
            ...
            const results = data.items.map((item: Record<string, unknown>, index: number) => ({
                position: start + index,
                title: (item.title as string) || 'No title',
                link: item.link as string,
                snippet: (item.snippet as string) || 'No snippet available',
    ...
    private async performImageSearch(args: ImageSearchArgs): Promise<RPCToolCallContent[]> {
        ...
        try {
            ...
            const results = data.items.map((item: Record<string, unknown>, index: number) => ({
                ...
                link: item.link as string,
                displayLink: item.displayLink as string,
                thumbnail: (item.image as { thumbnailLink?: string } | undefined)?.thumbnailLink,
                contextLink: (item.image as { contextLink?: string } | undefined)?.contextLink,
                width: (item.image as { width?: number } | undefined)?.width,
                height: (item.image as { height?: number } | undefined)?.height,
                size: (item.image as { byteSize?: number } | undefined)?.byteSize,
                snippet: (item.snippet as string) || 'No snippet available',
            }));
