...
export class FileSystemClient extends MCPClientLike {
    ...
    private async initializeDirectory(): Promise<void> {
        ...
        try {
            ...
            this.directoryHandle = await pickerWindow.showDirectoryPicker();
    ...
    async callTool(toolName: string, args: unknown): Promise<RPCToolCallContent[]> {
        ...
        try {
            switch (toolName) {
                case "fs_read_file":
                    return await this.readFile(parsedArgs.path ?? "");
                case "fs_write_file":
                    return await this.writeFile(parsedArgs.path ?? "", parsedArgs.content ?? "");
                case "fs_list_directory":
                    return await this.listDirectory(parsedArgs.path || "");
                case "fs_create_directory":
                    return await this.createDirectory(parsedArgs.path ?? "");
                case "fs_delete_file":
                    return await this.deleteFile(parsedArgs.path ?? "");
                case "fs_search_files":
                    return await this.searchFiles(parsedArgs);
                case "fs_copy_file":
                    return await this.copyFile(parsedArgs.source ?? "", parsedArgs.destination ?? "");
                case "fs_move_file":
                    return await this.moveFile(parsedArgs.source ?? "", parsedArgs.destination ?? "");
                case "fs_get_file_info":
                    return await this.getFileInfo(parsedArgs.path ?? "");
                case "fs_find_duplicates":
                    return await this.findDuplicates(parsedArgs.path || "", parsedArgs.byContent || false);
                case "fs_tree_view":
                    return await this.getTreeView(parsedArgs.path || "", parsedArgs.maxDepth || 5, parsedArgs.showHidden || false);
                default:
                    return [{
                        type: 'text',
    ...
    private async readFileAsPDF(file: File, _limit: number): Promise<RPCToolCallContent[]> {
    ...
    private async findDuplicates(path: string, byContent: boolean): Promise<RPCToolCallContent[]> {
        ...
        if (byContent) {
            ...
            for (const [_hash, paths] of hashMap) {
                if (paths.length > 1) {
