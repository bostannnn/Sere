...
export class ModuleHandler extends MCPToolHandler {
  ...
  async handle(toolName: string, args: Record<string, unknown>): Promise<RPCToolCallContent[] | null> {
    ...
    switch (toolName) {
      case 'risu-list-modules':
        return await this.listModules(a.count, a.offset)
      case 'risu-get-module-info':
        return await this.getModuleInfo(a.id ?? '', a.fields)
      case 'risu-set-module-info':
        return await this.setModuleInfo(a.id ?? '', a.data ?? {})
      case 'risu-list-module-lorebooks':
        return await this.listModuleLorebooks(a.id ?? '', a.count, a.offset)
      case 'risu-get-module-lorebook':
        return await this.getModuleLorebook(a.id ?? '', a.names ?? [])
      ...
      case 'risu-delete-module-lorebook':
        return await this.deleteModuleLorebook(a.id ?? '', a.name ?? '')
      case 'risu-get-module-regex-scripts':
        return await this.getModuleRegexScripts(a.id ?? '')
      ...
      case 'risu-delete-module-regex-script':
        return await this.deleteModuleRegexScript(a.id ?? '', a.name ?? '')
      case 'risu-get-module-lua-script':
        return await this.getModuleLuaScript(a.id ?? '')
      case 'risu-set-module-lua-script':
        return await this.setModuleLuaScript(a.id ?? '', a.code ?? '')
    }
    return null
  }
  ...
  async getModuleInfo(id: string, fields?: string[]): Promise<RPCToolCallContent[]> {
    ...
    for (const field of targetFields) {
      ...
      if (field === 'enabled') {
        ...
        const val = (module as unknown as Record<string, unknown>)[field]
        if (val !== undefined) {
          obj[field] = val
        }
      }
  ...
  async setModuleInfo(id: string, data: Record<string, unknown>): Promise<RPCToolCallContent[]> {
    const module = DBState.db.modules.find((m) => m.id === id)
    if (!module || module.mcp) {
      return [
        {
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      if (key === 'enabled') {
        ...
        // @ts-ignore -- key is constrained by allowedFields before dynamic module assignment.
        module[key] = value
      }
    }
  ...
  async setModuleLuaScript(id: string, code: string): Promise<RPCToolCallContent[]> {
    ...
    if (firstTrigger?.effect?.[0]?.type === 'triggerlua') {
      // @ts-ignore -- validated trigger structure contains `triggerlua` code when this branch is reached.
