      ...
      async (fieldsArg, targetIndex) => {
        ...
        const expected = Object.fromEntries(
          fields.map((field) => {
            ...
            return [field, target[field as keyof RisuModule]]
          })
        )
...
test('errs if module not found', async () => {
  ...
  expect(
    errors
      .map((responses) => responses?.[0] ?? null)
      .every((response) => (response as RPCToolCallTextContent | null)?.text === 'Error: Module with ID zzz not found.')
  ).toBe(true)
})
