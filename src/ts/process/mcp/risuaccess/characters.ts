...
export class CharacterHandler extends MCPToolHandler {
  ...
  async handle(toolName: string, args: Record<string, unknown>): Promise<RPCToolCallContent[] | null> {
    const a = args as {
      id?: string
      fields?: string[]
      count?: number
    ...
    switch (toolName) {
      case 'risu-get-character-info':
        return await this.getCharacterInfo(a.id ?? '', a.fields ?? [])
      case 'risu-list-character-lorebooks':
        return await this.getCharacterLorebooks(a.id ?? '', a.count, a.offset)
      case 'risu-get-character-lorebook':
        return await this.getCharacterLorebook(a.id ?? '', a.names ?? [])
      case 'risu-set-character-info':
        return await this.setCharacterInfo(a.id ?? '', a.data ?? {})
      case 'risu-set-character-lorebook':
        return await this.setCharacterLorebook(
          a.id ?? '',
          a.name ?? '',
          a.content ?? '',
          typeof a.keys === 'string' ? a.keys.split(',').map((v) => v.trim()).filter(Boolean) : [],
          a.newName,
          a.alwaysActive
        )
      case 'risu-delete-character-lorebook':
        return await this.deleteCharacterLorebook(a.id ?? '', a.name ?? '')
      case 'risu-get-character-regex-scripts':
        return await this.getCharacterRegexScripts(a.id ?? '')
      ...
      case 'risu-delete-character-regex-scripts':
        return await this.deleteCharacterRegexScripts(a.id ?? '', a.name ?? '')
      case 'risu-get-character-additional-assets':
        return await this.getCharacterAdditionalAssets(a.id ?? '')
      case 'risu-get-character-lua-script':
        return await this.getCharacterLuaScript(a.id ?? '')
      case 'risu-set-character-lua-script':
        return await this.setCharacterLuaScript(a.id ?? '', a.code ?? '')
      case 'risu-delete-character-additional-assets':
        return await this.deleteCharacterAdditionalAssets(a.id ?? '', a.assetName ?? '')
      case 'risu-list-characters':
        return await this.listCharacters(a.count, a.offset)
    }
    return null
  }
  ...
  async setCharacterInfo(id: string, data: Record<string, unknown>): Promise<RPCToolCallContent[]> {
    const char = getCharacter(id)
    if (!char) {
      return [
        {
    ...
    for (const [field, value] of Object.entries(data)) {
      if (fieldRemap[field as keyof typeof fieldRemap]) {
        const realField = fieldRemap[field as keyof typeof fieldRemap]
        // @ts-ignore -- dynamic key is validated against fieldRemap above before assignment.
