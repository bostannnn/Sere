...
export class AIAccessClient extends MCPClientLike {
    ...
    async callTool(methodName: string, args: unknown): Promise<RPCToolCallContent[]> {
        if (methodName === 'runLLM') {
            const payload = (typeof args === 'object' && args !== null ? args : {}) as {
                model?: string;
            ...
            const r = await requestChatData({
                formated: messages.map((msg) => ({
                    role: (msg.role === 'assistant' || msg.role === 'system' || msg.role === 'function') ? msg.role : 'user',
                    content: msg.content ?? ''
                })),
                bias: {}
