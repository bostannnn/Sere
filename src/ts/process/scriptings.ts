...
const scriptingLog = (..._args: unknown[]) => {};
...
type ScriptApiFunction = (...args: any[]) => any

function assertScriptPermission(allowed: boolean, scope: 'safe' | 'editDisplay' | 'lowLevel') {
    if (allowed) return
    throw new Error(`Script permission denied (${scope})`)
}
...
export async function runScripted(code:string, arg:{
    ...
    const initialChat = arg.chat ?? getCurrentChat()
    if (!initialChat) {
    ...
    return await ScriptingEngineState.mutex.runExclusive(async () => {
        ...
        if (code !== ScriptingEngineState.code) {
            let declareAPI:(name: string, func: ScriptApiFunction) => void = () => {
                throw new Error('Scripting API bridge initialization failed.')
            }

            if(ScriptingEngineState.type === 'lua'){
                scriptingLog('Creating new Lua engine for mode:', mode)
                ...
                declareAPI = (name:string, func: ScriptApiFunction) => {
                    luaEngine.global.set(name, func)
                }
            } else {
                scriptingLog('Creating new Pyodide context for mode:', mode)
            ...
            declareAPI('setChatVar', (id:string,key:string, value:string) => {
                assertScriptPermission(ScriptingSafeIds.has(id) || ScriptingEditDisplayIds.has(id), 'editDisplay')
            ...
            declareAPI('stopChat', (id:string) => {
                assertScriptPermission(ScriptingSafeIds.has(id), 'safe')
                stopSending = true
            })
            ...
            declareAPI('getChatLength', (_id:string) => {
                return ScriptingEngineState.chat.message.length
            })

            declareAPI('getFullChatMain', (_id:string) => {
            ...
            declareAPI('setFullChatMain', (id:string, value:string) => {
                assertScriptPermission(ScriptingSafeIds.has(id), 'safe')
                const realValue = JSON.parse(value) as Array<{ role: string; data: string }>

                ScriptingEngineState.chat.message = realValue.map((v: { role: string; data: string }) => {
                    return {
            ...
            declareAPI('getCharacterImageMain', async (_id:string) => {
                try {
                    ...
                    scriptingLog('Failed to create character image inlay')
                    return ''
                } catch (_error) {
                    scriptingLog('Error in getCharacterImageMain:', _error)
                    return ''
                }
            })

            declareAPI('getPersonaImageMain', async (_id:string) => {
                try {
                ...
                if(useMultimodal) {
                    for(const msg of promptbody) {
                        ...
                        for(const inlay of inlays) {
                            ...
                            if (!inlayData?.type || !inlayData?.data) {
                                continue
                            }
                            multimodals.push({
                                type: inlayData.type,
            ...
            declareAPI('getCharacterFirstMessage', (_id:string) => {
                const db = getDatabase()
                const selectedChar = get(selectedCharID)
                const char = db.characters[selectedChar]
            ...
            declareAPI('getPersonaName', (_id:string) => {
                return getUserName()
            })

            declareAPI('getPersonaDescription', (_id:string) => {
                const db = getDatabase()
                const selectedChar = get(selectedCharID)
                const char = db.characters[selectedChar]
            ...
            declareAPI('getAuthorsNote', (_id:string) => {
            ...
            declareAPI('getCharacterLastMessage', (_id: string) => {
            ...
            declareAPI('getUserLastMessage', (_id: string) => {
                const chat = ScriptingEngineState.chat
                if (!chat) {
                    return ''
                }
            ...
            scriptingLog('Running Lua code:', code)
            if(ScriptingEngineState.type === 'lua'){
        ...
        if(ScriptingEngineState.type === 'lua'){
            const luaEngine = ScriptingEngineState.engine
            if (!luaEngine) {
                throw new Error('Lua engine is not initialized.')
            }
            try {
                switch(mode){
                    case 'input':{
...
async function getOrCreateEngineState(
    ...
    const creationPromise = (() => {
        const engineState: ScriptingEngineState = {
            mutex: new Mutex(),
            type: type,
            chat: getCurrentChat() ?? createEmptyChat(),
            setVar: setChatVar,
            getVar: getChatVar,
        };
...
export async function runLuaEditTrigger<T extends string|OpenAIChat[]>(char:character|groupChat|simpleCharacterArgument, mode:string, content:T, meta?:object):Promise<T>{
    ...
    try {
        ...
        const charTriggers = Array.isArray((char as { triggerscript?: triggerscript[] }).triggerscript)
            ? (char as { triggerscript: triggerscript[] }).triggerscript
            : []
        const triggers = char.type === 'group' ? (getModuleTriggers()) : (charTriggers.map((v: triggerscript) => {
...
export async function runLuaButtonTrigger(char:character|groupChat|simpleCharacterArgument, data:string):Promise<any>{
    ...
    const charTriggers = Array.isArray((char as { triggerscript?: triggerscript[] }).triggerscript)
        ? (char as { triggerscript: triggerscript[] }).triggerscript
        : []
    const triggers = char.type === 'group' ? getModuleTriggers() : charTriggers.map<triggerscript>((v) => ({
        ...v,
...
class PyodideContext{
    worker: Worker;
    apis: Record<string, ScriptApiFunction> = {};
    ...
    constructor(){
        ...
        this.worker.onmessage = (event:MessageEvent) => {
            if(event.data.type === 'call'){
                ...
                if(this.apis[func]){
                    Promise.resolve(this.apis[func](...args)).then((result: unknown) => {
