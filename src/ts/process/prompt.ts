...
export interface PromptItemTyped {
    type: 'persona'|'description'|'lorebook'|'postEverything'|'rulebookRag'|'gameState'|'characterState'|'userState'|'relationshipState'|'semanticRecallCharacterState'|'semanticRecallUserState'|'semanticRecallRelationshipState'
    innerFormat?: string,
    name?: string
}
...
type STPresetLike = {
    prompts?: Array<{ identifier?: string | number; content?: string; role?: string }>
    prompt_order?: Array<{ order?: Array<{ enabled?: boolean; identifier?: string | number }> }>
    assistant_prefill?: string
}

export function stChatConvert(pre: STPresetLike): PromptItem[]{
    //ST preset
    const promptTemplate: PromptItem[] = []

    function findPrompt(identifier:number | string){
        return pre.prompts?.find((p) => p.identifier === identifier)
    }
    const normalizePromptRole = (role?: string): PromptItemPlain['role'] => {
    ...
    for(const prompt of pre?.prompt_order?.[0]?.order ?? []){
        ...
        if(p){
            ...
            promptLog("Prompt not found", prompt)
    ...
    for(let i=0;i<files.length;i++){
        ...
        const getParam = (setname:keyof(typeof preset), getname:string = '', arg:{
            ...
            if(OobaParams.includes(getname)){
                (oobaData as unknown as Record<string, unknown>)[getname] = data[getname]
            }
        }
            ...
            case 'PARAMETERS':{
                ...
                for(const key of OobaParams){
                    if(samplers.includes(key) && (data[key] !== undefined) && (data[key] !== null)){
                        (oobaData as unknown as Record<string, unknown>)[key] = data[key]
                    }
                }
                break
            }
