...
type CommandHandler = (arg: CommandHandlerArg) => Promise<string | false | undefined> | string | false | undefined
...
export function registerCommandHandler(name: string, handler: CommandHandler) {
    ...
    let set = commandHandlers.get(key)
    if(!set){
        set = new Set()
        commandHandlers.set(key, set)
    }
    set.add(handler)
}

export function unregisterCommandHandler(name: string, handler: CommandHandler) {
    const key = name.trim()
    const set = commandHandlers.get(key)
    if(!set){
        return
    }
    set.delete(handler)
    ...
    if(handlers && handlers.size > 0){
        for(const handler of handlers){
            try{
                ...
                commandLog('[CommandHandler] Error:', error)
                alertNormal(`${language.error}: ${error?.message ?? error}`)
            }
        }
    }
