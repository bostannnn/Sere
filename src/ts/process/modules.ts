...
export async function readModule(buf:Buffer):Promise<RisuModule | undefined> {
    let pos = 0
    ...
    if(readByte() !== 111){
        moduleLog("Invalid magic number")
        alertError(language.errors.noData)
        return
    }
    if(readByte() !== 0){ //Version check
        moduleLog("Invalid version")
        alertError(language.errors.noData)
        return
    }
...
export function getModuleMcps() {
    ...
    return modules.map((v) => v.mcp?.url).filter((v): v is string => !!v)
}
