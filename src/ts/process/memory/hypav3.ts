...
export interface HypaV3Settings {
  ...
  summarizationAllowThinking: boolean;
...
const hypaLog = (..._args: unknown[]) => {};

function getSummaryBatchSize(settings: HypaV3Settings): number {
...
async function runPeriodicSummarization(
  ...
  const setPeriodicDebug = (info: {
    ...
    db.hypaV3Debug.timestamp = Date.now();
    db.hypaV3Debug.model = resolvedModel;
    db.hypaV3Debug.isResummarize = false;
    db.hypaV3Debug.prompt = db.hypaV3Debug.prompt || settings.summarizationPrompt;
    db.hypaV3Debug.periodic = info;
  };
...
export async function hypaMemoryV3(
  ...
  try {
    if (settings.useExperimentalImpl) {
      hypaLog(logPrefix, "Using experimental implementation.");
...
async function hypaMemoryV3MainExp(
  ...
  if (!settings.preserveOrphanedMemory) {
    cleanOrphanedSummary(chats, data, room);
  }
  ...
  hypaLog(logPrefix, "Starting index:", startIdx);
  ...
  if (shouldReserveMemoryTokens) {
    ...
    hypaLog(logPrefix, "Reserved memory tokens:", memoryTokens);
  }
  ...
  while (summarizationMode) {
    ...
    const batchSize = getSummaryBatchSize(settings);
    while (
      toSummarize.length < batchSize &&
      ...
      if (chat.memo === "NewChat") {
        hypaLog(logPrefix, `Skipping new chat at index ${currentIndex}`);
        shouldSummarize = false;
      }

      if (chat.content.trim().length === 0) {
        hypaLog(logPrefix, `Skipping empty chat at index ${currentIndex}`);
        shouldSummarize = false;
      }

      if (settings.doNotSummarizeUserMessage && chat.role === "user") {
        hypaLog(logPrefix, `Skipping user role at index ${currentIndex}`);
        shouldSummarize = false;
      }
  ...
  if (toSummarizeArray.length > 0) {
    ...
    rateLimiter.taskQueueChangeCallback = (_queuedCount) => {
    ...
    for (let i = 0; i < batchResult.results.length; i++) {
      ...
      if (!result.success || !result.data) {
        ...
        hypaLog(logPrefix, "Summarization failed:", `\n${errorMessage}`);
  ...
  if (settings.similarMemoryRatio > 0) {
    ...
    processor.progressCallback = (current, total) => {
      hypaV3ProgressStore.set({
        open: true,
        miniMsg: `${total - current}`,
        msg: `${logPrefix} Embedding summaries...`,
        subMsg: `${current} / ${total} embedded`,
      });
    };
...
async function hypaMemoryV3Main(
  ...
  if (shouldReserveEmptyMemoryTokens) {
    ...
    hypaLog(logPrefix, "Reserved empty memory tokens:", emptyMemoryTokens);
  } else {
    currentTokens += memoryTokens;
    hypaLog(logPrefix, "Reserved max memory tokens:", memoryTokens);
  }
  ...
  while (summarizationMode) {
    ...
    for (let i = startIdx; i < endIdx; i++) {
      ...
      if (
        ...
        hypaLog(logPrefix, `Skipping example chat at index ${i}`);
        continue;
      }

      if (chat.memo === "NewChat") {
        hypaLog(logPrefix, `Skipping new chat at index ${i}`);
        continue;
      }

      if (chat.content.trim().length === 0) {
        hypaLog(logPrefix, `Skipping empty chat at index ${i}`);
        continue;
      }

      if (settings.doNotSummarizeUserMessage && chat.role === "user") {
        hypaLog(logPrefix, `Skipping user role at index ${i}`);
        continue;
      }
...
function cleanOrphanedSummary(
  ...
  // Prefer full chat memos from room history to avoid trimming-valid summaries.
  const roomMemos =
    room?.message
      ?.map((msg) => msg.chatId)
  ...
  if (removedCount > 0) {
    hypaLog(logPrefix, `Cleaned ${removedCount} orphaned summaries.`);
  }
}
...
function stripSummaryForPrompt(content: string): string {
...
export async function summarize(oaiMessages: OpenAIChat[], isResummarize: boolean = false): Promise<string> {
  const db = getDatabase();
  const settings = getCurrentHypaV3Preset().settings;
  const previousPeriodic = db.hypaV3Debug?.periodic;
  ...
  // API
  if (settings.summarizationModel === "subModel") {
    hypaLog(logPrefix, `Using ax model ${db.subModel} for summarization.`);
    ...
    db.hypaV3Debug.rawResponse = response.result;
  ...
  const content = await chatCompletion(formated, settings.summarizationModel, {
    ...
    temperature: 0,
    extra_body: {
      enable_thinking: !!settings.summarizationAllowThinking,
    },
  });
  ...
  db.hypaV3Debug.rawResponse = content;
...
export function createHypaV3Preset(
  ...
  // Current product direction: keep HypaV3 summarization on auxiliary model.
  settings.summarizationModel = "subModel";
...
function _simpleRRF<T>(rankedLists: T[][], k: number = 60): T[] {
  const scores = new Map<T, number>();
...
function _normalizeScores<T>(scoredList: [T, number][]): [T, number][] {
