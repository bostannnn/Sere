...
const SUPA_PUNCTUATION_REGEX = /[.,/#!$%&*;:{}=_`~()^-]/g;
            ...
            if(arg.asHyper && (!id.startsWith("hypa:"))){
                ...
                if(id.startsWith("hypa:")){
                    ...
                    if(db.removePunctuationHypa){
                        supaMemory = supaMemory.replace(SUPA_PUNCTUATION_REGEX, "")
                    }
        ...
        if(arg.asHyper){
            ...
            if(hypaChunks.length > 0){
                await hypa.addText(hypaChunks.filter((value, index, self) => {
                    ...
                    if(db.removePunctuationHypa){
                        value = value.replace(SUPA_PUNCTUATION_REGEX, "")
                    }
                    return value
                }).filter((v) => {
                    return !supaMemory.replace(SUPA_PUNCTUATION_REGEX, "").includes(v.replace(SUPA_PUNCTUATION_REGEX, ""))
                }))
