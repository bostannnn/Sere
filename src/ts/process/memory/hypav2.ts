...
async function summary(
    ...
    if (db.supaModelType !== "subModel") {
        ...
        hypaLog("Using openAI instruct 3.5 for SupaMemory");
        ...
        hypaLog("Using submodel: ", db.subModel, "for supaMemory model");
...
function isOldHypaV2Data(obj: unknown): obj is OldHypaV2Data {
    ...
    return (
        ...
        data.mainChunks.every(mainChunk =>
...
export function regenerateSummary(
    ...
    const _targetMainChunk = data.mainChunks[mainChunkIndex];
    void chats;
    void _targetMainChunk;
...
export async function hypaMemoryV2(
    ...
    if (room.hypaV2Data) {
        ...
        hypaLog("Old HypaV2 data detected. Converting to new format...");
...
// Token management loop (If current token usage exceeds allowed amount)
    while (currentTokens > maxContextTokens) {
        ...
        while (
            ...
            if (idx === 0) {
                hypaLog("[HypaV2] Skipping index 0");
                idx++;
                continue;
            }
            ...
            if (!chat.content.trim()) {
                hypaLog(`[HypaV2] Skipping empty content of index ${idx}`);
                idx++;
                continue;
            }
        ...
        if (!summaryData.success) {
            hypaLog("Summarization failed:", summaryData.data);
            summarizationFailures++;
            if (summarizationFailures >= maxSummarizationFailures) {
                hypaLog("[HypaV2] Summarization failed multiple times. Aborting...");
                return {
                    currentTokens: currentTokens,
                    chats: chats,
        ...
        hypaLog("[HypaV2] tokens after summarization deduction:", currentTokens);
    }
