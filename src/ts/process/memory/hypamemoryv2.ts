...
const hypaV2ProcessorLog = (..._args: unknown[]) => {};
...
type VectorCacheStore = {
  getItem<T>(key: string): Promise<T | null>;
  setItem<T>(key: string, value: T): Promise<T>;
};

const hypaV2MemoryCache = new Map<string, unknown>();

function createVectorCacheStore(): VectorCacheStore {
  if (isNodeServer) {
    return {
      async getItem<T>(key: string): Promise<T | null> {
        return hypaV2MemoryCache.has(key)
          ? (hypaV2MemoryCache.get(key) as T)
          : null;
      },
      async setItem<T>(key: string, value: T): Promise<T> {
        hypaV2MemoryCache.set(key, value);
        return value;
      },
    };
  }
  ...
  return {
    async getItem<T>(key: string): Promise<T | null> {
      const value = await forage.getItem<T>(key);
      return value ?? null;
    },
    async setItem<T>(key: string, value: T): Promise<T> {
...
export class HypaProcessorV2<TMetadata> {
  ...
  public progressCallback: (current: number, total: number) => void = null;
  public vectors: Map<string, EmbeddingResult<TMetadata>> = new Map();
  private forage: VectorCacheStore = createVectorCacheStore();
  ...
  public cancel(): void {
    this.canceled = true;
    this.options.rateLimiter.cancelPendingTasks("User canceled embedding");
  }
  ...
  private async getEmbeds(
    ...
    if (this.isLocalModel()) {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        // Progress callback (report BEFORE processing chunk to show initial state)
        this.progressCallback?.(processedItems, toEmbed.length);
      ...
      if (!this.canceled) {
        this.progressCallback?.(toEmbed.length, toEmbed.length);
      }
    } else {
      // API model: Parallel processing
      let processedItems = 0;
      this.progressCallback?.(0, toEmbed.length);

      const embeddingTasks = chunks.map((chunk) => {
        const contents = chunk.map((item) => item.content);

        return async () => {
          if (this.canceled) throw new TaskCanceledError("User canceled embedding");
          const result = await this.getAPIEmbeds(contents);
          processedItems += chunk.length;
          this.progressCallback?.(processedItems, toEmbed.length);
          return result;
        };
  ...
  private async getLocalEmbeds(contents: string[]): Promise<EmbeddingVector[]> {
    if (isNodeServer) {
      ...
      const vectors = (response.data as { vectors?: EmbeddingVector[] })?.vectors;
