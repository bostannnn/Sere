<script lang="ts">
    ...
    const chatBodyLog = (..._args: unknown[]) => {};
        ...
        try {
            ...
            if(retranslate || translated){
                if (DBState.db.showTranslationLoading) {
                    lastParsed = `<div class="ds-chat-translate-loading"><div class="ds-chat-translate-loading-spinner"></div></div>`
                }
    ...
    const checkImg = () => {
        ...
        if (imgs && imgs.length > 0) {
            imgs.forEach(async (img) => {
                ...
                chatBodyLog('Checking image:', name, 'Assets:', assets)
    ...
    let markParsingResult = $state<Promise<string>>(Promise.resolve(''))

    $effect(() => {
        const parseResult = markParsing(msgDisplay, character, idx)
        markParsingResult = parseResult
        checkImg()
        parseResult.then(checkImg)
    })
</script>
