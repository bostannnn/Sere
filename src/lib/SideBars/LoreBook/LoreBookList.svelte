<script lang="ts">
    ...
    const loreBookListLog = (..._args: unknown[]) => {};
    ...
    let sorted = $state(0)
    const idgroup = 'a' + v4() //make should it starts with alphabetic character
    ...
    const recreateStb = async () => {
        ...
        try {
            ...
            loreBookListLog('DOM stabilization failed:', error);
        ...
        if (openedDetails === 0) {
            try {
                ...
                loreBookListLog('Failed to recreate sortable:', error);
                // Retry
                await sleep(50);
                try {
                    createStb();
                } catch (retryError) {
                    loreBookListLog('Retry failed:', retryError);
                }
            }
        }
    }
    ...
    const createStb = () => {
        stb = Sortable.create(ele, {
            ...
            onEnd: async (evt) => {
                ...
                const sortedArray = [];
                const processedItems = new SvelteSet();
    ...
    let openedRefs = new SvelteSet<unknown>() // Track both folders + lorebooks (for UI state)
    
    // Derived state to calculate number of open folders
    const openFolders = $derived(() => {
        let count = 0
    ...
    const onOpen = (isDetail: boolean = true, bookRef?: unknown) => {
        if (isDetail) {
    ...
    const onClose = (isDetail: boolean = true, bookRef?: unknown) => {
        if (isDetail) {
...
{#key sorted}
    <div
        class="lorebook-list-root list-shell"
        data-testid="lorebook-list-shell"
        bind:this={ele}
    ...
    >
        ...
        {:else if externalLoreBooks}
            ...
            {#if externalLoreBooks.length === 0}
                <span class="lorebook-list-empty empty-state" data-testid="lorebook-list-empty">No Lorebook</span>
            {:else}
                {#each externalLoreBooks as book, i (book)}
        ...
        {:else if submenu === 0}
            ...
            {#if DBState.db.characters[$selectedCharID].globalLore.length === 0}
                <span class="lorebook-list-empty empty-state" data-testid="lorebook-list-empty">No Lorebook</span>
            {:else}
                {#each DBState.db.characters[$selectedCharID].globalLore as book, i (book)}
                    {#if (!showFolder && !book.folder) || (showFolder === book.folder)}
        ...
        {:else if submenu === 1}
            ...
            {#if DBState.db.characters[$selectedCharID].chats[DBState.db.characters[$selectedCharID].chatPage].localLore.length === 0}
                <span class="lorebook-list-empty empty-state" data-testid="lorebook-list-empty">No Lorebook</span>
            {:else}
                {#each DBState.db.characters[$selectedCharID].chats[DBState.db.characters[$selectedCharID].chatPage].localLore as book, i (book)}
                    {#if (!showFolder && !book.folder) || (showFolder === book.folder)}
