...
export function getFirstMessage(): string | null {
  ...
  if (typeof chat.fmIndex !== "number" || chat.fmIndex < 0) {
    return null;
  }

  return char.alternateGreetings?.[chat.fmIndex] ?? null;
}
...
export function getCategoryName(categoryId: string | undefined, categories: Array<{ id?: string; name?: string }>): string {
  const category = categories.find(c => c.id === (categoryId || ""));
  return category?.name || language.memoryModal.unclassified;
}
...
export function shouldShowSummary(summary: unknown): boolean {
