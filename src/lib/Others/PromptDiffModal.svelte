<script lang="ts">
    ...
    const { firstPresetId, secondPresetId, onClose = () => {} }: Props = $props();
...
// -----------------------------------------------------------------------------
    const pillBase = 'ds-prompt-diff-pill seg-tab'
    const pillActive = 'ds-prompt-diff-pill-active'
    const pillInactive = 'ds-prompt-diff-pill-inactive'
    const diffLineBase = 'ds-prompt-diff-line'
    const diffLineCommon = 'ds-prompt-diff-line-delta'

    const diffSameClass = 'ds-prompt-diff-line-same'
    const diffAddClass = `${diffLineCommon} ds-prompt-diff-line-add`
    const diffRemoveClass = `${diffLineCommon} ds-prompt-diff-line-remove`

    const tokenAddClass = 'ds-prompt-diff-token-add'
    const tokenRemoveClass = 'ds-prompt-diff-token-remove'
    const tokenSameClass = 'ds-prompt-diff-token-same'

    const lineRemoveClass = 'ds-prompt-diff-line-delta ds-prompt-diff-line-remove'
    const lineAddClass    = 'ds-prompt-diff-line-delta ds-prompt-diff-line-add'
    const lineModifyClass = 'ds-prompt-diff-line-delta ds-prompt-diff-line-modify'

    const nameHeaderTagClass = 'ds-prompt-diff-tag'
    const controlLabelClass = 'ds-prompt-diff-control-label'
    const controlLabelDimClass = 'ds-prompt-diff-control-label ds-prompt-diff-control-label-dim'
    const emptyLineNoteClass = 'ds-prompt-diff-empty-line'

    const metaLineRemoveClass = `${diffLineBase} ds-prompt-diff-meta-remove`
    const metaLineAddClass = `${diffLineBase} ds-prompt-diff-meta-add`
    const metaLineRemoveSoftClass = `${diffLineBase} ds-prompt-diff-meta-remove-soft`
    const metaLineAddSoftClass = `${diffLineBase} ds-prompt-diff-meta-add-soft`
    const splitEmptyLineClass = `${diffLineBase} ${diffLineCommon} ds-prompt-diff-line-empty-split`

    const tokenPackLineAdd: TokenClassPack = {
      add: 'ds-prompt-diff-token-line-add',
      remove: '',
      same: 'ds-prompt-diff-token-line-add-same',
    }

    const tokenPackLineRemove: TokenClassPack = {
      add: '',
      remove: 'ds-prompt-diff-token-line-remove',
      same: 'ds-prompt-diff-token-line-remove-same',
    }
...
// -----------------------------------------------------------------------------
    ...
    async function computeDiffCardLine(prompt1: PromptCard[], prompt2: PromptCard[], _style: DiffStyle): Promise<CardDiffSummary> {
...
{#snippet pillRadioGroup(label: string, name: string, options: readonly { value: string; label: string }[], value: string, setValue: (v: string) => void, disabled = false)}
  <div class="ds-prompt-diff-control-row">
    <span class={controlLabelClass}>{label}</span>
    <div class="ds-prompt-diff-pill-group seg-tabs">
      {#each options as opt (opt.value)}
        <label
          class={`${pillBase} ${value === opt.value ? pillActive : pillInactive} ${
                  disabled ? 'ds-prompt-diff-pill-disabled' : ''
          }`}
        >
          <input
            class="ds-prompt-diff-radio-hidden"
            type="radio"
            {name}
            value={opt.value}
            {disabled}
...
{#snippet checkboxToggle(label: string, checked: boolean, setChecked: (v: boolean) => void, disabled = false, dimWhenDisabled = false)}
  <div class="ds-prompt-diff-control-row">
    <label
      class={`ds-prompt-diff-check-label ${
        disabled && dimWhenDisabled ? 'ds-prompt-diff-check-label-dim' : ''
      }`}
    >
      <input
        type="checkbox"
        class="ds-prompt-diff-checkbox-input"
        {disabled}
        checked={checked}
...
{#snippet rangeControl(label: string, value: number, setValue: (v: number) => void, min = 0, max = 5, disabled = false)}
  <div class="ds-prompt-diff-control-row">
    <span class={disabled ? controlLabelDimClass : controlLabelClass}>
      {label}
    </span>
    <input
      type="range"
      ...
      step="1"
      value={value}
      class="ds-prompt-diff-range-input"
      {disabled}
      oninput={(e) => {
        if (disabled) return
    ...
    />
    <span
      class={`ds-prompt-diff-range-value ${
        disabled ? 'ds-prompt-diff-range-value-dim' : ''
      }`}
    >
      {value}
    </span>
...
{#snippet renderCounts(counts: DiffCounts)}
  <div class="ds-prompt-diff-counts">
    <span class="ds-prompt-diff-count-item">
      <span class="ds-prompt-diff-count-bar ds-prompt-diff-count-bar-modified"></span>
      {counts.modifiedCount}
    </span>
    <span class="ds-prompt-diff-count-item">
      <span class="ds-prompt-diff-count-bar ds-prompt-diff-count-bar-added"></span>
      {counts.addedCount}
    </span>
    <span class="ds-prompt-diff-count-item">
      <span class="ds-prompt-diff-count-bar ds-prompt-diff-count-bar-removed"></span>
      {counts.removedCount}
    </span>
  </div>
{/snippet}
...
{#snippet renderSimpleLine(part: NonModifyPart)}
  <div
    ...
    class:ds-prompt-diff-block-gap-top={isNameLine(part)}
    class:ds-prompt-diff-block-gap-bottom={isHeaderLine(part)}
  >
...
{#snippet renderModify(part: ModifyPart, idx: number)}
  ...
  {#if diffStyle === 'line'}
    <!-- line diff -->
    <div
      class={`${diffLineBase} ${lineRemoveClass}`}
      class:ds-prompt-diff-block-gap-top={isLineby(part) && part.right.lineRole === 'name' && idx > 0}
    >
      {@render renderTokens(part.tokens, 'add', tokenPackLineRemove)}{#if tag}<span class={nameHeaderTagClass}>{tag}</span>{/if}
    </div>

    <div
      class={`${diffLineBase} ${lineAddClass}`}
      class:ds-prompt-diff-block-gap-bottom={isLineby(part) && part.right.lineRole === 'header'}
    >
      {#if part.src === 'linebyline' && part.right.text === ''}
        <span class={emptyLineNoteClass}>[empty line]</span>
      {:else}
  ...
  {:else} <!-- intraline diff -->
    <div
      class={`${diffLineBase} ${lineModifyClass}`}
      class:ds-prompt-diff-block-gap-top={isLineby(part) && part.right.lineRole === 'name' && idx > 0}
      class:ds-prompt-diff-block-gap-bottom={isLineby(part) && part.right.lineRole === 'header'}
    >
...
{#snippet renderModifyGroup(parts: ModifyPart[])}
  {#if diffStyle === 'line'}
    <!-- grouped + line diff -->
    <div class={`${diffLineBase} ${lineRemoveClass}`}>
    ...
    <div class={`${diffLineBase} ${lineAddClass}`}>
    {#each parts as part, i (i)}
      {#if part.src === 'linebyline' && part.right.text === ''}
        <span class={emptyLineNoteClass}>[empty line]</span>
      {:else}
...
{#snippet renderCardMeta(part: DiffPart, type: string, side: Side | null)}
  <div class="ds-prompt-diff-meta">
    <span class="ds-prompt-diff-meta-title">{type}</span>
    ...
    {#if part && part.src === 'linebyline'}
      {#if part.k === 'modify'}
        {#if diffStyle === 'line'}
          ...
          {#if side === null}
            <!-- unified -->
            <div class={metaLineRemoveClass}>
              {@render renderTokens(part.tokens, 'add', tokenPackLineRemove)}
            </div>
            <div class={metaLineAddClass}>
              {@render renderTokens(part.tokens, 'remove', tokenPackLineAdd)}
            </div>
          {:else} <!-- split -->
            <div class={side === 'left' ? metaLineRemoveClass : metaLineAddClass}>
              {@render renderTokens(
                part.tokens,
        ...
        {:else}
          ...
          {#if side === null}
            <!-- unified intraline -->
            <div class={diffLineBase}>
              {@render renderTokens(part.tokens, null, tokenPackIntraline)}
            </div>
          {:else}
            <!-- split intraline: -->
            <div class={side === 'left' ? metaLineRemoveSoftClass : metaLineAddSoftClass}>
              {@render renderTokens(
                part.tokens,
                side === 'left' ? 'add' : 'remove',
                tokenPackIntraline
      ...
      {:else}
        <!-- same/add/remove -->
        <div
          class={`ds-prompt-diff-line ${
            part.k === 'same'
              ? 'ds-prompt-diff-meta-same'
              : part.k === 'add'
              ? 'ds-prompt-diff-meta-add'
              : 'ds-prompt-diff-meta-remove'
          }`}
        >
          {part.line.text}
        </div>
      {/if}
    {:else}
      <div class="ds-prompt-diff-empty-inline">No {type}</div>
    {/if}
  </div>
{/snippet}

{#snippet renderDivider(d: Extract<DiffSegment, { kind: 'divider' }>, scope: string)}
  <div class="ds-prompt-diff-divider-row">
    <div class="ds-prompt-diff-divider-line"></div>
    <button
      type="button"
      class={`ds-prompt-diff-divider-button ${d.omitted > 0 ? 'is-active' : 'is-disabled'}`}
...
{#snippet renderSplitCell(cell: SplitCell, idx: number)}
  {@const role = cell.role}
    {#if cell.kind === 'empty'}
      <div
        ...
        class:ds-prompt-diff-block-gap-top={role === 'name' && idx > 0}
        class:ds-prompt-diff-block-gap-bottom={role === 'header'}
      >
        &nbsp;
      </div>
  ...
  {:else}
    ...
    {:else}
      ...
      <div
        class={`${diffLineBase} ${isLeft ? lineRemoveClass : lineAddClass}`}
        class:ds-prompt-diff-block-gap-top={role === 'name' && idx > 0}
        class:ds-prompt-diff-block-gap-bottom={role === 'header'}
      >
...
{#snippet renderCardStatus(cardPart: CardBodyDiff)}
  ...
  {@const statusClass =
    cardPart.k === 'modify' ? 'ds-prompt-diff-status-chip-modified'
    : cardPart.k === 'add'  ? 'ds-prompt-diff-status-chip-added'
    : cardPart.k === 'remove' ? 'ds-prompt-diff-status-chip-removed'
    : 'ds-prompt-diff-status-chip-unchanged'}

  <div class="ds-prompt-diff-status">
    <span class={`ds-prompt-diff-status-chip control-chip ${statusClass}`}>
      {statusLabel}
    </span>
    <span class="ds-prompt-diff-status-meta">
...
<div class="ds-prompt-diff-overlay">
  <div class="ds-prompt-diff-modal">
    
    <div class="ds-prompt-diff-header">
      <div class="ds-prompt-diff-toolbar">
      ...
      <button
        type="button"
        class="ds-prompt-diff-close icon-btn icon-btn--sm"
        onclick={() => {handleClose()}}
        title="Close prompt diff"
        aria-label="Close prompt diff"
      >
        <XIcon size={20}/>
      </button>
    </div>

    <div class="ds-prompt-diff-body">
      <!-- card view -->
      {#if !isFlatText && formatStyle === 'card'}
        {#if cardDiffResult}
          ...
          <div class="ds-prompt-diff-summary-row">
            {@render renderCounts(cardDiffResult.counts)}
            <div class="ds-prompt-diff-summary-meta">
              <span>Cards changed:</span>
              <span class="ds-prompt-diff-summary-value">{cardChangedTotal}</span>
              <span class="ds-prompt-diff-summary-divider">/</span>
              <span>compared {cardDiffResult.parts.length}</span>
              <span class="ds-prompt-diff-summary-divider">·</span>
              <span>total {firstCards.length} → {secondCards.length}</span>
              <span class="ds-prompt-diff-summary-divider">·</span>
              <span class="ds-prompt-diff-summary-accent-modified">
                ~{cardDiffResult.cardCounts.modifiedCount}
              </span>
              <span class="ds-prompt-diff-summary-divider">/</span>
              <span class="ds-prompt-diff-summary-accent-added">
                +{cardDiffResult.cardCounts.addedCount}
              </span>
              <span class="ds-prompt-diff-summary-divider">/</span>
              <span class="ds-prompt-diff-summary-accent-removed">
                -{cardDiffResult.cardCounts.removedCount}
              </span>
            </div>
          </div>

          <div class="ds-prompt-diff-card-grid">
            {#each visibleCardParts as cardPart, idx (idx)}
              ...
              <div class="ds-prompt-diff-card">
                <!-- name / header / card diff -->
                <div class="ds-prompt-diff-card-head">
                  <div class="ds-prompt-diff-card-meta">
                ...
                <!-- body -->
                <div class="ds-prompt-diff-card-body">
                  {#if bodyParts.length === 0}
                    <div class="ds-prompt-diff-empty-inline">No body content</div>
                  {:else}
              ...
              {:else} <!-- split view -->
                ...
                <div class="ds-prompt-diff-split-shell">
                  <div class="ds-prompt-diff-split-bg">
                    <div class="ds-prompt-diff-split-bg-pane"></div>
                    <div class="ds-prompt-diff-split-bg-pane"></div>
                  </div>

                  <div class="ds-prompt-diff-split-grid">
                    <!-- left header -->
                    <div class="ds-prompt-diff-split-head-cell">
                      ...
                      {:else}
                        <div class="ds-prompt-diff-split-head-empty">
                          <span class="ds-prompt-diff-split-head-empty-label">-</span>
                        </div>
                      {/if}
                    </div>
                    ...
                    <div class="ds-prompt-diff-split-midline"></div>
                    <div class="ds-prompt-diff-split-midline"></div>

                    <!-- body rows -->
                    {#each rows as r, rIdx (r.key)}
                      {#if r.kind === 'divider'}
                        <div class="ds-prompt-diff-split-divider-cell">
                          {@render renderDivider(r.seg, r.scope)}
                        </div>
                      {:else}
                        <div class="contents ds-prompt-diff-row-group">
                        <div class="ds-prompt-diff-split-row-cell">
        ...
        {:else}
          <div class="ds-prompt-diff-empty-note">No diff computed yet.</div>
        {/if}
      {:else}<!-- raw view -->
        {#if currentFlatResult}
          ...
          {#if showOnlyChanges && segments.length === 0}
            <div class="ds-prompt-diff-nochange-wrap">
              <div class="ds-prompt-diff-nochange-badge control-chip">
                <span class="ds-prompt-diff-nochange-dot"></span>
                <span class="ds-prompt-diff-nochange-label">No changes</span>
              </div>
            </div>
          {:else if viewStyle === 'unified' || isFlatText}
          <div class="ds-prompt-diff-mono-block">
            {#each segments as seg, sIdx (sIdx)}
              {#if seg.kind === 'divider'}
          ...
          {:else} <!-- split view -->
            ...
            <div class="ds-prompt-diff-raw-split">
              <div class="ds-prompt-diff-raw-split-grid">
                {#each rows as r, idx (r.key)}
                  {#if r.kind === 'divider'}
                    <div class="ds-prompt-diff-raw-divider-cell">
                      {@render renderDivider(r.seg, r.scope)}
                    </div>
                  {:else}
                    <div class="contents ds-prompt-diff-row-group">
                    <div class="ds-prompt-diff-raw-row-cell">
                      {@render renderSplitCell(r.left, idx)}
                    </div>
                    <div class="ds-prompt-diff-raw-col-divider"></div>
                    <div class="ds-prompt-diff-raw-row-cell">
