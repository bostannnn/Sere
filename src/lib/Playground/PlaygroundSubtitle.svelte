...
<script lang="ts">
    ...
    let mode = $state('llm')
    const sourceLang:string|null = $state(null)    
    const playgroundSubtitleLog = (..._args: unknown[]) => {};
    ...
    async function runWhisperMode() {
        ...
        if(mode === 'whisperLocal'){
            try {
                ...
                playgroundSubtitleLog(outputText)
...
<div class="playground-subtitle-root panel-shell">
...
<SelectInput bind:value={mode} onchange={() => {
    if(mode === 'llm'){
...
{:else if vttB64 && fileB64}
    <details class="mt-4 list-shell">
        <pre>{outputText}</pre>
    </details>
{:else}
    <pre class="list-shell">{outputText}</pre>
{/if}
