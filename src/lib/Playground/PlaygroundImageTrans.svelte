<script lang="ts">
    ...
    const playgroundImageTransLog = (..._args: unknown[]) => {};
    ...
    async function imageTranslate(_type:number = 0) {
        if(loading){
            return;
        }
        loading = true;
        try {
            ...
            if(mode === 'manual'){
                let outputObj:Array<Record<string, unknown>> = []
                playgroundImageTransLog(d.result)
                playgroundImageTransLog(jsonOutputTrimmer(d.result))
                ...
                playgroundImageTransLog(outputObj)
    ...
    async function render() {
        ...
        playgroundImageTransLog('rendered')
...
<div class="playground-image-trans-root panel-shell">
<SelectInput bind:value={mode} className="w-1/2 control-field">
...
<div class="relative mt-2 list-shell">
    <canvas bind:this={canvas} class:blur-effect={loading && mode === 'auto'} onpointerdown={(e) => {
        ...
        onpointermove={(e) => {
            if(mode === 'manual' && mouseDown){
                ...
                playgroundImageTransLog(selection.style.left, selection.style.top, selection.style.width, selection.style.height);
            }
        }}
