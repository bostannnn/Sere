<script lang="ts">
  ...
  // For revoking
  let previewURLs = new SvelteMap<string, string>()
...
<div class="playground-inlay-root panel-shell">
  <h2 class="text-4xl text-textcolor mt-6 font-black relative">Inlay Asset Explorer</h2>

  <header class="flex flex-wrap gap-4 py-6 items-center sticky top-0 bg-bgcolor action-rail">
  ...
  {#if allAssets.length === 0 && !loading}
    <div class="text-center py-12 text-textcolor2 empty-state">
  ...
  {:else}
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {#each displayedAssets as [id, asset] (id)}
          {#key selection.has(id)}
            <div class="border border-darkborderc rounded-lg p-4 bg-darkbg panel-shell">
    ...
    {#if hasMore}
      <div class="my-6 text-center action-rail">
