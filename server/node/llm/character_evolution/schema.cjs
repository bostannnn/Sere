...
const DEFAULT_EXTRACTION_PROMPT = [
    ...
    '- If a detail is new but not yet durable, keep the current state unchanged and mention it only in lastInteractionEnded or keyMoments if relevant.',
    ...
    '- If maybe, prefer lastInteractionEnded or keyMoments.',
    ...
    '- status: corrected = superseded by newer evidence and not current',
    '- status: archived = history only and not current',
    '- confidence: suspected = weak or first-pass evidence',
    '- confidence: likely = moderate support',
    '- confidence: confirmed = explicit, repeated, or clearly durable',
    '',
    'Field discipline:',
    ...
    '- lastInteractionEnded: use this for fresh scene residue, unresolved mood, and immediate carry-forward details that are too recent or too narrow for durable sections',
    '',
...
const BUILTIN_SECTION_DEFS = [
    ...
    { key: 'userRead', label: 'User Read', instruction: 'Track {{char}}\'s durable subjective interpretation of {{user}} only. Add only if clearly shown by {{char}}\'s words, stance, or repeated framing in this chat. Do not log momentary interpretations or scene-specific emotional readings.', kind: 'list', includeInPrompt: true },
    ...
    { key: 'lastInteractionEnded', label: 'Last Interaction Ended', instruction: 'Describe only how the current processed interaction range ended and what should carry into the next one. Use this for fresh scene residue, unresolved mood, and immediate carry-forward details that are too recent or too narrow for durable sections. Do not summarize the whole conversation here.', kind: 'object', includeInPrompt: true },
