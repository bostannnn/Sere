...
const CONFLICT_CONFIDENCE_RANK = {
    suspected: 0,
    likely: 1,
    confirmed: 2,
};

const ARCHIVE_ON_RESOLUTION_SECTIONS = new Set([
    'activeThreads',
    'runningJokes',
    'keyMoments',
]);

const SLOT_REPLACEMENT_SECTIONS = new Set([
    'userFacts',
    'activeThreads',
]);
...
function normalizeConflictValue(valueRaw) {
...
function analyzeConflictItem(item) {
...
function getConflictPreferencePolarity(tokens) {
...
function countCommonSuffix(left, right, prefixCount) {
...
function countSharedMeaningfulTokens(left, right) {
...
function getRemainingMeaningfulTokensAfterPrefix(analysis, commonPrefixCount) {
    return filterMeaningfulConflictTokens(analysis.tokens.slice(commonPrefixCount));
}

function getRemainingMeaningfulTokensAfterPrefixAndSuffix(analysis, commonPrefixCount, commonSuffixCount) {
...
function oneSpanLooksLikeRefinement(left, right) {
    if (left.length === 0 || right.length === 0) {
        return false;
    }
...
function findTokenSequenceIndex(tokens, sequence) {
...
function getComparisonConflictPattern(tokens) {
...
function isShortLocationFactFrame(tokens) {
...
function shouldArchiveConflict(sectionKey, left, right) {
...
function shouldCorrectByFactSlotReplacement(sectionKey, left, right) {
...
function shouldCorrectByPreferenceComparisonReversal(sectionKey, left, right) {
...
function shouldCorrectByOppositePreferencePolarity(sectionKey, left, right) {
    if (!COMPARISON_CONFLICT_SECTIONS.has(sectionKey)) {
        return false;
    }
...
function shouldCorrectConflict(sectionKey, left, right) {
...
function getConflictTransition(sectionKey, leftItem, rightItem) {
...
function getConfidenceRank(item) {
    return item?.confidence ? CONFLICT_CONFIDENCE_RANK[item.confidence] : -1;
}

function pickWinningProposalIndex(items, leftIndex, rightIndex) {
...
function pickBestProposalConflictIndex(items, candidateIndexes) {
...
function pickWinningReinforcementProposalIndex(items, leftIndex, rightIndex) {
...
function pickBestProposalReinforcementIndex(items, candidateIndexes) {
    return candidateIndexes.reduce((bestIndex, candidateIndex) => {
        if (bestIndex < 0) {
            return candidateIndex;
        }
...
function resolveConflictsWithinProposal(sectionKey, itemsRaw) {
    const items = clone(itemsRaw);
...
function resolveCurrentAgainstProposal(sectionKey, currentItemsRaw, proposedItemsRaw) {
    const proposedItems = clone(proposedItemsRaw);
    const currentActiveItems = (Array.isArray(currentItemsRaw) ? currentItemsRaw : [])
        .filter((item) => getItemStatus(item) === 'active');
    ...
    for (const currentItem of currentActiveItems) {
        ...
        if (conflictingIndexes.length === 0) {
            carryForwardItems.push(clone(currentItem));
            continue;
        }
...
function resolveCharacterEvolutionSectionConflicts(arg = {}) {
    const resolvedProposal = resolveConflictsWithinProposal(
        arg.sectionKey,
        Array.isArray(arg.proposedItems) ? arg.proposedItems : []
    );
    return resolveCurrentAgainstProposal(
        arg.sectionKey,
        Array.isArray(arg.currentItems) ? arg.currentItems : [],
        resolvedProposal
    );
}
...
function resolveCharacterEvolutionStateConflicts(arg = {}) {
    ...
    for (const key of CHARACTER_EVOLUTION_ITEM_SECTION_KEYS) {
        ...
        nextState[key] = resolveCharacterEvolutionSectionConflicts({
            ...
            proposedItems: Array.isArray(nextState[key]) ? nextState[key] : [],
        });
    }
...
module.exports = {
    ...
    resolveCharacterEvolutionStateConflicts,
};
