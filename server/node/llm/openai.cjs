...
function buildExecutionRequest(input, settings, arg = {}) {
    ...
    if (!Array.isArray(requestBody.messages) || requestBody.messages.length === 0) {
        throw new LLMHttpError(
            400,
            'INVALID_MESSAGES',
            'OpenAI request requires non-empty messages.'
        );
    }

    requestBody.stream = !!input.streaming;

    const apiKey = typeof settings?.openAIKey === 'string' ? settings.openAIKey.trim() : '';
    if (!apiKey && requireKey) {
        throw new LLMHttpError(400, 'OPENAI_KEY_MISSING', 'OpenAI key is not configured on server settings.');
    }
...
async function previewOpenAIExecution(input, ctx = {}) {
...
async function executeOpenAI(input, ctx = {}) {
    const settings = await loadServerSettings(ctx.dataRoot);
    ...
    if (!parsed || typeof parsed !== 'object') {
        throw new LLMHttpError(502, 'INVALID_UPSTREAM_RESPONSE', 'OpenAI returned non-JSON response.');
    }

    const result = parseResponseText(parsed);
    if (typeof result !== 'string') {
        throw new LLMHttpError(502, 'INVALID_UPSTREAM_RESPONSE', 'OpenAI response did not contain text content.');
    }
