...
import { registerMemoryTraceRoutes } from "./trace_routes.cjs";
import { normalizePromptOverride, applyPromptOverride } from "./prompt_override.cjs";
...
function buildHandlers(arg: {
  ...
  registerMemoryTraceRoutes({
    ...
    toLLMErrorResponse,
    resolveMemorySettings: () => ({ summarizationPrompt: "Prompt", doNotSummarizeUserMessage: false }),
    convertStoredMessageForMemorySummary: () => null,
    buildMemorySummarizationPromptMessages: () => [],
    resolveMemorySummaryProviderModel: () => ({ provider: "test", model: "test" }),
    buildMemoryTraceResponsePayload: () => ({ type: "success" }),
    normalizeMemoryDataForEdit: () => ({ summaries: [] }),
    sanitizeMemorySummarizationContent: (text: string) => text,
    planPeriodicMemorySummarization: () => ({ shouldRun: false, reason: "not_ready", promptMessages: [] }),
...
describe("memory trace routes", () => {
  it("returns early when auth fails", async () => {
    ...
    const handler = handlers.get("/data/memory/manual-summarize/trace");
    if (!handler) {
    ...
    await handler(
      {
        method: "POST",
        originalUrl: "/data/memory/manual-summarize/trace",
  ...
  it("returns early for periodic trace when auth fails", async () => {
    ...
    const handler = handlers.get("/data/memory/periodic-summarize/trace");
    if (!handler) {
    ...
    await handler(
      {
        method: "POST",
        originalUrl: "/data/memory/periodic-summarize/trace",
