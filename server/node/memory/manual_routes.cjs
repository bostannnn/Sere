function registerMemoryManualRoutes(arg = {}) {
    const {
        app,
        fs,
        dataDirs,
    ...
    if (!app || typeof app.post !== 'function') {
        throw new Error('registerMemoryManualRoutes requires an Express app instance.');
    }
    if (typeof safeResolve !== 'function') {
        throw new Error('registerMemoryManualRoutes requires safeResolve.');
    }
    if (typeof normalizePromptOverride !== 'function'
        || typeof applyPromptOverride !== 'function'
        || typeof resolveManualPromptSource !== 'function') {
        throw new Error(
            'registerMemoryManualRoutes requires normalizePromptOverride/applyPromptOverride/resolveManualPromptSource.'
        );
    }
    ...
    async function persistChatViaStateCommand(characterId, chatId, chat) {
        if (typeof applyStateCommands !== 'function') {
            throw new LLMHttpError(
                ...
                'Internal state command service is unavailable for memory manual summarize.',
            );
        }
    ...
    const handler = async (req, res) => {
        ...
        const endpoint = 'memory_manual_summarize';
        ...
        try {
            ...
            const summaryText = await executeMemorySummaryFromMessages({
                settings,
            ...
            const memoryData = normalizeMemoryDataForEdit(getMemoryData(chat));
            ...
            const debugPayload = {
                ...
                prompt: memorySettings.summarizationPrompt,
            ...
            await appendLLMAudit({
                ...
                response: buildMemoryAuditResponsePayload(endpoint, successPayload),
            });
    ...
    app.post('/data/memory/manual-summarize', handler);
}
