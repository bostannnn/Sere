...
import { registerMemoryManualRoutes } from "./manual_routes.cjs";
import {
...
function createReq(body: Record<string, unknown>): MockReq {
  return {
    method: "POST",
    originalUrl: "/data/memory/manual-summarize",
    body,
  };
}
...
function resolveMemorySettings(
  settings: Record<string, unknown>,
  character: Record<string, unknown> | null,
) {
  const presetId = Number(settings?.memoryPresetId ?? 0);
  const presets = Array.isArray(settings?.memoryPresets) ? settings.memoryPresets : [];
  ...
  const characterPrompt = toStringOrEmpty(
    (character?.memoryPromptOverride as { summarizationPrompt?: unknown } | undefined)?.summarizationPrompt,
  ).trim();
...
function buildHandler(): RegisteredHandler {
  ...
  registerMemoryManualRoutes({
    ...
    convertStoredMessageForMemorySummary: (message: { role?: string; data?: unknown; memo?: unknown }) => {
    ...
    executeMemorySummaryFromMessages: async (arg: { meta?: { provider?: string | null; model?: string | null } }) => {
      if (arg?.meta) {
    ...
    normalizeMemoryDataForEdit: (raw: unknown) => {
  ...
  const handler = postHandlers.get("/data/memory/manual-summarize");
  if (!handler) {
...
async function invokeWithAuthBlocked(body: Record<string, unknown>) {
  ...
  registerMemoryManualRoutes({
    ...
    executeMemorySummaryFromMessages: async () => "Stub summary output",
    generateSummaryEmbedding: async () => null,
    normalizeMemoryDataForEdit: (raw: unknown) => {
      const base = (raw && typeof raw === "object" && !Array.isArray(raw)) ? raw as Record<string, unknown> : {};
      const summaries = Array.isArray(base.summaries) ? base.summaries : [];
...
beforeEach(() => {
  tmpRoot = mkdtempSync(path.join(os.tmpdir(), "memory-manual-routes-test-"));
  dataDirs = {
    root: tmpRoot,
...
describe("memory manual summarize route", () => {
