...
import { registerHypaV3ResummaryRoutes } from "./hypav3_resummary_routes.cjs";
...
function buildHandlers(arg: {
  ...
  registerHypaV3ResummaryRoutes({
    ...
    getReqIdFromResponse: () => "req-resummary-test",
    toStringOrEmpty,
    ...
    toLLMErrorResponse,
    resolveHypaV3Settings: () => ({ reSummarizationPrompt: "Prompt" }),
    ...
    executeHypaSummaryFromMessages: async () => "summary",
  ...
  return {
    preview: postHandlers.get("/data/memory/hypav3/resummarize-preview"),
    apply: postHandlers.get("/data/memory/hypav3/resummarize-apply"),
  };
}

describe("hypav3 resummary routes", () => {
  it("returns early when auth fails", async () => {
    ...
    if (!preview) {
      throw new Error("preview handler not registered");
    }
    const res = createRes();
    await preview(
      {
        method: "POST",
        originalUrl: "/data/memory/hypav3/resummarize-preview",
  ...
  it("returns early for apply when auth fails", async () => {
    const handlers = buildHandlers({
      requirePasswordAuth: (_req, res) => {
        res.statusCode = 401;
        res.payload = { error: "UNAUTHORIZED", message: "Unauthorized" };
    ...
    if (!apply) {
      throw new Error("apply handler not registered");
    }
    const res = createRes();
    await apply(
      {
        method: "POST",
        originalUrl: "/data/memory/hypav3/resummarize-apply",
        body: {
          characterId: "charA",
          chatId: "chatA",
          summaryIndices: [0, 1],
  ...
  it("returns INVALID_PATH for preview when safeResolve rejects path", async () => {
    const handlers = buildHandlers({
      safeResolve: () => {
  ...
  it("returns INVALID_PATH for apply when safeResolve rejects path", async () => {
    const handlers = buildHandlers({
      safeResolve: () => {
        throw new Error("bad path");
      },
